---
title: "BM + IST + Asymptotic Safety — v6.3"
subtitle: "Self-Critically Revised Tripartite Research Program — External Red-Team Pending"
...
**Revision status:** self-critically revised from v6.2. The supplied adversarial review was AI-generated and is therefore not independent external validation. External red-team review remains a required milestone and is not yet completed.

**Current claim:** the program is a disciplined research program for testing whether a discrete arithmetic substrate can yield the observed relativistic quantum regime through a controlled scale-dependent effective description.
...
### 1.3.1 Why arithmetic remains the incumbent hypothesis

The arithmetic commitment is a research bet, not a protected conclusion. It is retained because the incumbent substrate must simultaneously support **discreteness, a nontrivial invariant-measure problem, scale-sensitive coarse-graining, and an ultrametric/place structure** without importing a continuum Hilbert space at the microscopic level. Arithmetic dynamics provides an unusually compact hypothesis class in which these questions can be posed with existing mathematics.

That argument does **not** imply that non-arithmetic discrete systems cannot support canonical measures or quantum-like effective sectors. They can. The program therefore uses a finite null budget rather than an unlimited search. The initial classes are: (1) a globally algebraic degree-2 map with an explicit $p=2$ local realization; (2) the analogous $p=3$ class; (3) a multi-place $p=2\times3$ construction if single-place structure fails; and (4) a non-arithmetic symbolic/graph-dynamical control of comparable finite complexity. Any further class requires an explicit pre-registered reason.

The arithmetic layer is abandoned if it cannot outperform this finite null budget by producing a distinctive, non-retrofitted bridge or prediction.
...
Two additional debt classes are promoted in v6.3:
...
For the current program, the independent reviewer must be **a different model family and/or a named human researcher**, not the same model instance that constructed the claim. Reviews are versioned artifacts. If owner and reviewer disagree, a third-party reviewer adjudicates the disputed debt. A promoted claim can be unpromoted by the reviewer, but only with a written reason tied to a defined debt class.
...
The most important discipline of v6.3 is to keep mathematical objects in the spaces where they actually live.
...
### 5.2 Prototype program for $\Phi$

The state-to-theory map is not allowed to remain a slogan. A first prototype is
...
where $\mathcal C_s$ is an explicit coarse-graining/blocking map and $\mathcal A_s$ reconstructs an effective-action equivalence class from a pre-registered finite family of observable correlators or moments. The prototype must test the commutation condition

$$
\Phi_{s'}\!\left(\mathcal C_{s\to s'}x\right) \simeq \mathfrak R_{s\to s'}\!\left(\Phi_s(x)\right),
$$

with an explicit tolerance or an exact theorem. Failure means the bridge is not faithful at that level; it does not justify changing the definition until agreement appears.
...
This gate is mandatory. Before constructing a bespoke physical map, run a **compatibility library** of simple, well-understood arithmetic dynamical examples. The purpose is not to call any example physical, but to determine whether the required properties can coexist on one map class at all. Start with the smallest globally algebraic degree-2 examples for which canonical-height theory is explicit and test whether the same map class, or a rigorously related local realization, also admits the required symbolic/Markov structure. If the pre-registered library contains no joint example, the compatibility claim becomes an obstruction rather than an indefinitely deferred construction.
...
This is the major new elevation in v6.3.
...
## 9.3 Chaos-tameness / tame-factor ladder

A crucial mathematical constraint sharpens the amplitude problem. For a genuinely mixing measure-preserving system, the Koopman operator has no non-constant eigenfunctions. Therefore the program should **not** search for the quantum phase as an ordinary point-spectrum sector of the same mixing dynamics that is being used to explain statistical relaxation.

The working theorem target is instead a factorization

$$
\pi_{\rm tame}:(X,F,\mu_*)\to(Y,G,\nu_*),
\qquad
\pi_{\rm tame}\circ F=G\circ\pi_{\rm tame},
$$

where $G$ is zero-entropy, equicontinuous, or odometer-type and can support a nontrivial unit-circle character group. In the prime-power case, the natural candidate is the $p^\infty$ root-of-unity spectrum of the adding-machine class, which connects directly to the p-adic additive-character workbench.

The sharpened ladder is therefore:

**T1:** invariant measure and transfer/Koopman objects exist.

**T2:** a nontrivial tame/Kronecker factor exists in the dynamical limit.

**T3:** the factor supports the required $U(1)$ phase structure, potentially through $\chi_p$.

**T4:** the factor closes with the amplitude modulus into a physically adequate complex amplitude and generator.

The cheap Phase-2 diagnostic is a **Kronecker/point-spectrum computation on the smallest admissible approximants**, followed by a projective/infinite-limit consistency test. Finite approximants necessarily have discrete spectra; the test is not merely whether an eigenvalue appears at finite $L$, but whether a nontrivial factor survives coherently as $L\to\infty$. If the limiting system is weakly mixing and no tame factor exists, the amplitude route is killed for that substrate.

This turns three formerly parallel ideas—p-adic characters, symbolic/Markov coding, and arithmetic admissibility—into a single hypothesis: they may be different representations of one tame factor if the construction exists.
...
For settings $a,b$, write the finite-resolution residual as $\delta\rho_{a,b}(x_A,x_B)$. No-signaling requires the marginals to be independent of the remote setting:

$$
\Delta_A(x_A;a,b)=\int \delta\rho_{a,b}(x_A,x_B)\,dx_B,
$$

with

$$
\Delta_A(x_A;a,b)=\Delta_A(x_A;a),
\qquad
\Delta_B(x_B;a,b)=\Delta_B(x_B;b),
$$

for all allowed settings, together with the normalization/baseline condition needed to recover the declared equilibrium marginal. Any nontrivial residual may therefore live only in joint correlators, not in controllable marginal changes.

If a correction permits controllable signaling for any settings family, the theory fails regardless of its other virtues.
...
A local scale is permitted only if derived from the flow and shown to preserve recovery and scheme-independence. The question remains a **debt item**, not a primitive.

### 14.8 AS redundancy test

The Occam rule is symmetric: if the dFRG machinery contributes no distinctive, scheme-stable quantity or theorem that a simpler effective-flow construction cannot supply, AS-specific machinery must be removed rather than kept for prestige.
...
Freeze, before construction begins: (i) the finite substrate-class budget, (ii) the no-signaling residual family indexed by measurement settings, (iii) the **Valentini fork** as a theory-level experimental claim family, and (iv) the null models to be compared later: standard BM typicality, BM+Valentini without the arithmetic layer, a non-arithmetic discrete substrate, and a plain-AS/direct-effective-flow control.
...
The first physical construction is deliberately narrow. Select the simplest pre-registered arithmetic class capable of supporting both the algebraic and dynamical requirements; the default first case is a **globally algebraic degree-2 self-map with an explicit $p=2$ local realization**. The formula is not chosen because it looks quantum; it is chosen because it is the smallest class that can be audited without hiding assumptions.

**Deliverable A:** state explicitly $X_L$, the encoding, $F_L$, $L$, and the induced placewise action.

**Deliverable B:** canonical-height applicability report, including the exact global hypotheses actually satisfied.

**Deliverable C:** Markov/symbolic applicability report on the **same map**, not a separate convenience model.

**Deliverable D:** transfer/Koopman diagnostics and the first tame-factor/Kronecker-spectrum test.

**Deliverable E:** prime-universality scan and p-adic-character closure test.

**Deliverable F:** explicit non-arithmetic null substrate of comparable finite complexity.

This is not a claim that $p=2$ is physical. It is a finite-search starting point. If the default class fails, the pre-registered budget determines the next class; the program does not widen the class indefinitely.

### Phase-1 kill

If $F_L$ cannot be constructed faithfully, stop the arithmetic branch. If $F_L$ exists but fails the joint requirements without a defensible replacement, do not silently replace it with a more convenient map.

## Phase 2 — Tameness and measure

The theorem shelf is branch-specific:

- **Deterministic branch:** transfer/Perron-Frobenius operators, Lasota-Yorke-type estimates where applicable, ergodic decomposition, and Kronecker-factor theory.
- **Post-trace stochastic branch:** random dynamical systems, strong-Feller/asymptotic-strong-Feller methods, invariant-measure and mixing theorems under their actual hypotheses.

The phase must not cite a stochastic RDS theorem as though it applied to the deterministic pre-trace map.
...
Construct and run the pre-registered null models in parallel. A prediction is informative only if it differs from at least one credible simpler account.
...
After the finite substrate-class budget and null models are exhausted, remove the arithmetic layer if it produces no distinctive, held-out leverage.
...
Remove AS-specific machinery if a simpler non-AS construction reproduces the same outputs without loss of predictive content.
...
If EBP creates more process cost than debt-retirement value, simplify the protocol while preserving its anti-promotion function. The protocol itself is not sacred.
...
# 25. Subtraction Record: What v6.3 Adds, Restores, or Reclassifies
...
- the trace-relative versus autonomous hidden-sector/dark-matter branch; no dark-sector phenomenology is part of the core until the quantum core and trace construction are complete.
...
| Tame/Kronecker factor exists | TARGET / GATE | Koopman/transfer analysis |
...
4a. The limiting substrate is weakly mixing with no nontrivial tame factor, and no alternative amplitude construction survives.
...
18a. The finite substrate-class budget is exhausted without a map class that survives the joint arithmetic/tameness tests.
19. AS contributes no distinctive quantitative or mathematical leverage.
19a. A null model reproduces every retained prediction while being structurally simpler.
...
# 30. The Elephant and the End State

The Blind Men and the Elephant analogy is the epistemic constitution of the program. BM, IST, and AS are not entitled to be the elephant merely because each illuminates a genuine structural feature. The elephant is whatever survives translation, adversarial testing, and subtraction.

The program therefore asks the symmetric question:

> **What remains if BM, IST, AS, E8, and even this research program are all allowed to be wrong?**

A successful outcome should be structurally sparse: a small microscopic rule set, controlled coarse-graining, a derived measure, an emergent complex amplitude sector, a quantum IR generator, and the least additional structure required for relativistic and gravitational recovery.

The program is successful even when the answer is negative, provided the failure is localized, reproducible, and reduces the hypothesis space.

**Final doctrine:** construct before interpretation; type before identification; pre-register before comparison; separate equilibrium destination from relaxation path; compare null models symmetrically; and let failure remove structure rather than add it.
...
- C. Wetterich, foundational functional-renormalization-group work; methodological ancestry for effective-average-action flows.
- M. Reuter, foundational asymptotic-safety fixed-point work; benchmark for fixed-point/critical-surface reasoning.
- R. Bowen and standard symbolic-dynamics references on Markov partitions.
- J. Tate, foundational harmonic analysis on local fields and additive characters.
- K. Falconer and standard fractal-geometry references for dimension and regularity.
- B. O. Koopman, foundational operator-theoretic treatment of dynamical systems; used for the point-spectrum diagnostic.
- Standard ergodic-theory references on Kronecker factors, weak mixing, and spectral decomposition.

---

# Appendix B — What v6.3 Corrects Relative to v6.2

Relative to v6.2, this revision specifically:
...
- strengthens the self-critical status statement: external red-team has not yet occurred;
- gives the first microscopic map a pre-registered construction class and finite search budget;
- turns the amplitude-factor gate into an explicit tame/Kronecker-factor problem;
- adds the limiting-spectrum criterion so finite approximant eigenvalues cannot be mistaken for a physical phase sector;
- gives $\Phi$ a concrete coarse-graining plus effective-action reconstruction prototype and an RG-commutation test;
- makes the no-signaling constraint setting-indexed rather than a single fixed correction;
- schedules null-model construction alongside phenomenology;
- names deterministic versus stochastic theorem shelves;
- restores the hidden/dark-sector branch to the subtraction record;
- and makes the arithmetic-class commitment finite-budget and falsifiable rather than indefinitely extensible.
